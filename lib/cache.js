...
async cacheReleaseList(url) {
  const { token, url: serverUrl } = this.config
  ...
  // browser_download_url redirects and drops the auth header
  const apiUrl = url
  ...
  const response = await retry(
    async () => {
      const res = await fetch(url, {
        headers: {
          ...headers,
          // Tell GitHub to redirect with token intact
  ...
  let content = await convertStream(response.body)
  ...
  for (let i = 0; i < matches.length; i += 1) {
    ...
    const proxyUrl = `${serverUrl}/update/win32/${this.latest.version}/${matches[i]}`
    console.error('Replacing:', matches[i], '→', proxyUrl)
    content = content.replace(matches[i], proxyUrl)
  }
  console.error('Final RELEASES content:', content)  // ← and this
  return content
}
