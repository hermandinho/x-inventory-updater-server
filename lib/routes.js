...
exports.releasesNupkg = async (req, res) => {
  const { filename } = req.params
  const latest = await loadCache()

  if (!latest || !latest.platforms) {
    send(res, 404, 'No release found')
    return
  }

  // Find asset matching the requested filename
  const asset = Object.values(latest.platforms).find(a => a.name === filename)

  if (!asset) {
    send(res, 404, `Asset ${filename} not found`)
    return
  }

  if (shouldProxyPrivateDownload) {
    proxyPrivateDownload(asset, req, res)
    return
  }

  res.writeHead(302, { Location: asset.url })
  res.end()
}
